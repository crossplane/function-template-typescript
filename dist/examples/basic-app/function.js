import { Resource, RunFunctionRequest, RunFunctionResponse, fatal, normal, setDesiredComposedResources, to, getDesiredComposedResources, getObservedCompositeResource, } from "function-sdk-typescript";
...
export class Function {
    async RunFunction(req, logger) {
        ...
        try {
            ...
            if (ingressConfig.enabled) {
                const ingress = new Ingress({
                    ...
                    spec: {
                        ...
                        ...(ingressConfig.hosts && {
                            rules: ingressConfig.hosts.map((hostConfig) => ({
                                host: hostConfig.host,
                                http: {
                                    paths: hostConfig.paths.map((pathConfig) => ({
            ...
            logger?.info({ duration: `${duration}ms`, resourceCount: Object.keys(dcds).length }, "Function completed successfully");
